...
void display_init()
{
	...
	uint8_t aidemo[] =    "  Toy Doll ";
	uint8_t aidemo2[] =   " Recognition";
	display_show_string(60U,  250, 200, 32, 32, mcxn9);
	display_show_string(60U,  290, 200, 32, 32, aidemo);
	display_show_string(60U,  330, 200, 32, 32, aidemo2);
}
...
void display_show_slice(uint32_t g_stripe_index, uint32_t buffer, uint32_t max_idx)
{
    ...
    if(g_stripe_index +1 == max_idx){
		...
		if(g_detedted_flag){
			...
			display_show_string( 32 + 0, 390, 280,32,32, (uint8_t *)"                            ");
			display_show_string( 32 + 0, 390, 280,32,32, (uint8_t *)str_buf);
		}
		else
		{
			display_show_string( 32 + 0, 390, 280,32,32, (uint8_t *)"  Unrecognized               ");
		}
